...
module.exports = function(context, opts) {
  ...
  ['keepFnName', 'simplify', 'undefinedToVoid'].forEach(optionName => {
    if (opts[optionName] !== undefined && typeof opts[optionName] !== 'boolean') {
      throw new Error(`Preset optimizations '${optionName}' option must be a boolean.`);
    }
  });

  const keepFnName = opts.keepFnName !== undefined ? opts.keepFnName : true;
  const simplify = opts.simplify !== undefined ? opts.simplify : true;
  const undefinedToVoid = opts.undefinedToVoid !== undefined ? opts.undefinedToVoid : true;

  return {
    plugins: [
      pluginMinifyConstantFolding,
      [pluginMinifyDeadCodeElimination, { keepFnName, optimizeRawSize: true }],
